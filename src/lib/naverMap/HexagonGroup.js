...
export class HexagonGroup {
   ...
   * @param {object} meta - 사용자가 임의로 추가할 수 있는 meta 데이터
   * @param {function} eventListener - focus, blur의 이벤트를 전달할 콜백 함수
   *
   * @return {void} 반환값 없음
   */
  constructor({
    ...
    this.listeners = [];
    this.meta = { ...meta };
    this.eventListenerCallback = eventListener;
  }
      ...
      if (this.infoHtml) {
        ...
        this.listeners = addEventListeners(
          ...
          () => {
            ...
            console.log('onClick');
            this.eventListenerCallback({ event: 'click', meta: this.meta });
          },
          // onMouseover
          () => {
            /* eslint-disable no-console */
            ...
            this.focus();
            this.eventListenerCallback({ event: 'mouseover', meta: this.meta });
          },
          // onMouseout
          () => {
            /* eslint-disable no-console */
            ...
            this.blur();
            this.eventListenerCallback({ event: 'mouseout', meta: this.meta });
          },
        );
      }
   ...
   * @return {void} 반환값 없음
   */
  blur() { // TODO 현재 focus인지 blur인지 상태 관리 로직이 필요(갯수가 많아지면 리소스 낭비가 발생하게 됨)
    if (!this.hexGroupPolygon) throw new Error('polygon 객체가 없습니다.');
