...
describe('#2 typeNumber', () => {
  ...
  test('#2-2 숫자 이외의 값을 입력하면 무시', async () => {
    // 1. [EVENT] 사용자가 유효하지 않은 값(문자)을 입력합니다.
    notValidValue = 'notValidValue';
    vTextField.$emit('change', notValidValue);
    await flushPromises();
    // 2. [CHECK] 사용자가 입력한 값이 표시되지 않아야 합니다.
    ...
    // TODO 유효하지 않는 몇가지 케이스 추가
  });

  test('#2-3 숫자를 입력하면 표시되어야 한다.', async () => {
    // 1. [EVENT] 사용자가 유효한 값을 입력합니다.
    notValidValue = 123456789;
    vTextField.$emit('change', notValidValue);
    await flushPromises();
    // 2. [CHECK] 사용자가 입력한 값이 표시되어야 합니다.
