<template>
  ...
  >
    ...
    <!-- 2. Center child hexagon의 h3Index -->
    <!-- 6. local J -->
    <ControlPanelRow
      label="Center Child"
      :value="h3IndexCenterChild"
    />
  </GeoToH3>
</template>
...
<script>
import {
  h3ToCenterChild, // https://h3geo.org/docs/api/hierarchy/#h3tocenterchild
...
export default {
  ...
  methods: {
    ...
    updateOverlays() {
      ...
      this.h3IndexCenterChild = h3ToCenterChild(this.h3IndexParent, this.h3IndexResolutionChildren);

      // 3. Center Child의 hexagon을 업데이트한다
      const overlayCenterChild = hexagonHandler.createHexagon({
        h3Index: this.h3IndexCenterChild,
      });

      // 4. 업데이트한 hexagon을 부모에게 emit 한다
      this.destroyOverlays();
      this.overlays = [overlayParent, overlayCenterChild];
